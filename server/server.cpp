	...
	while(true){
		if(selector.wait()){
			...
			if(packetid == PACKET_CONNECT){
				...
				player_t *newplayer = new player_t(PLAYER_PACKET_ID++, 0,0, username);
				newplayer->setAddress(&socket, client, clientPort);
				players.push_back(newplayer);
				newplayer->sendPacket(response);
				...
				for(player_t *player : players){
					...
					newplayer->sendPacket(addPlayerX);
				}
				...
				player->sendPacket(forceResponsePacket);
